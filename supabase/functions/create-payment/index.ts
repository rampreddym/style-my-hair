...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { appointmentId, amount, serviceName, stylistName, tip } = await req.json();
    ...
    const origin = req.headers.get("origin") || "https://hair-halo-hubs.lovable.app";
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: serviceName,
              description: stylistName ? `Service by ${stylistName}` : undefined,
            },
    ...
    // Update appointment with payment intent info
